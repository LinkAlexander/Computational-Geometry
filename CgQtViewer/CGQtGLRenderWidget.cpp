...
void CgQtGLRenderWidget::render(CgBaseRenderableObject* obj,glm::mat4 mv_matrix,glm::mat4 proj_matrix)
{
  ...
  setUniformValue("projMatrix",proj_matrix);
  setUniformValue("modelviewMatrix",mv_matrix);
  setUniformValue("normalMatrix",normal_matrix);

  //std::cout << (int)m_lighting << std::endl;
...
void CgQtGLRenderWidget::init(CgBaseRenderableObject* obj)
{
    ...
    {
        bobj->initTriangleMeshFlatShading((CgBaseTriangleMesh*)obj);

    }
    if(obj->getType()==Cg::HalfEdgeTriangleMesh)
    {
        bobj->initHalfEdgeTriangleMesh((CgBaseHalfEdgeTriangleMesh*)obj);
    }
...
void CgQtGLRenderWidget::initializeGL()
{
    ...
    m_trackball->init();
    m_trackball->place(glm::vec3(0.0,0.0,0.0),1.0);
